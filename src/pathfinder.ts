...
export async function pathfinder(
  ...
  // Layer 0 rides the import graph (Amendment 3 — index-primary): the
  // generated src/layer0.ts statically imports every layer0 file, so every
  // consumer's `deno install` materializes the whole tree — no protocol
  // checks, no degraded mode, works identically under file: and https:.
  // The tree files remain the source of truth; `gen index --check
  // src/layer0` keeps the index honest.
