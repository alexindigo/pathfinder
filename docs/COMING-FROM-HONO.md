...
| `c.req.json()`                                | `request.body.json()` — lazy accessor; `parseJson(request)` wraps parse failures into a catchable `ParseError`                                                                                       |
...
| `c.status()` / `c.body()` / `return c.json()` | return values coerce: object → JSON, string → text/plain, `Response` → identity, stream → streamed. Helpers: `json()`, `html()`, `text()`, `redirect()`                                              |
