...
Deno.test("factory: Layer 0 rows carry index identities (Amendment 3)", async () => {
  await quiet(async () => {
    const app = await pathfinder({ roots: [APP] });

    // End-of-chain proof: a loopback request to /_status SERVES the manifest,
    // and the served rows name the packaged tree files.
    ...
    assertEquals(status404Row.file, "layer0/404.ts"); // tree-relative path
    assertEquals(status404Row.layer, 0);
    assert(status404Row.root.endsWith("/src/"), status404Row.root); // checkout
    ...
    assertEquals(statusGetRow.file, "layer0/_status/get.ts");
    assertEquals(statusGetRow.layer, 0);

    // app.manifest() agrees with what the file served.
    assert(app.manifest().some((r) => r.file === "layer0/404.ts"));
  });
});
